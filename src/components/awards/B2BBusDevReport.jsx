...
export default function B2BBusDevReport({ award, recompeteDays = 180, subscriberName = null, oipId = null, onUpdateStatus = null, onPursue = null }) {
  ...
  // Shaped for ScoreExplainBadge via the shared helper -- see
  // src/lib/scoreDisplay.js for why this is factored out (avoids the same
  // display drift ScoreExplainBadge.jsx itself was built to prevent).
  const compositeScore = buildDDv2CompositeScore(scores, isCapitalBuild);
    ...
    return (
      <div className="wq-awards wq-report">
        ...
        {/* THE ACCOUNT — thin strip only. Depth (addressable book by code,
            ...
            account is not. */}
        {(book.awards != null || book.total != null || bd.incumbent_uei || ((bd.sub_agency || bd.agency) && bd.naics_code)) && (
          <Section title="The account">
            ...
            {((bd.sub_agency || bd.agency) && bd.naics_code) && (
              <p style={{ marginTop: "0.4rem" }}>
                <a
                  href={`/co-landscape?agency=${encodeURIComponent((bd.sub_agency || bd.agency).replace(/[.,]/g, ""))}&naics=${encodeURIComponent(bd.naics_code)}${bd.agency_poc?.name ? `&co=${encodeURIComponent(bd.agency_poc.name)}` : ""}`}
                  target="_blank"
                  rel="noopener noreferrer"
                  style={{ fontSize: "0.85rem", color: "#2563eb", textDecoration: "none" }}
                >
