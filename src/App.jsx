  ...
  return (
    <>
      <div className="hero" style={{ marginBottom: 16 }}>
        <div className="hero-eyebrow">Market Review</div>
      ...
      {loading ? <SectionLoader /> : (
        <>
          ...
          {filtered.length === 0 ? (
            <EmptyMessage title="No signals match your filters" message="Adjust filters above or try a different status." />
          ) : (
            <div>
              {filtered.map(s => (
                <SignalCard
...
function SentinelPage() {
  ...
  const loadSentinel = async () => {
    const { data: s } = await supabase
      .from('sentinels')
      .select('id, version, is_active, mode, match_fields, groups')
      .eq('oip_id', selectedOip.id)
      .eq('is_active', true)
      .single()
    setSentinel(s)
  ...
  return (
    <div className="detail-section">
      ...
      <h2 className="detail-title">Keyword vocabulary<HelpIcon topic="sentinel" /></h2>
